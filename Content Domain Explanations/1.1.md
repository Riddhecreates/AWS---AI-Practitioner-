...
Example: A rule-based chatbot is AI but not ML; A spam filter is ML; Face recognition uses deep learning; ChatGPT-style systems are Generative AI.
...
Example: Credit card fraud detection → real-time inferencing; Monthly demand forecasting → batch inferencing
...
Example: Sales records → structured, tabular data; Customer reviews → unstructured, text data; Security camera footage → unstructured, image/video data
...
Example: Email spam detection → supervised learning; Customer segmentation → unsupervised learning; Game-playing AI or robotics → reinforcement learning
