...
Evaluating an ML model requires both technical performance metrics and business metrics. Model performance metrics measure how well the model predicts outcomes, while business metrics measure whether the model delivers real value to the organization. A model can score high technically but still fail commercially. In the exam, AWS often checks whether you can choose the right metric for the use case, not just the most popular one.

*Performance metrics*
*Accuracy*

Accuracy measures the percentage of correct predictions out of all predictions made. It is simple and intuitive but can be misleading when classes are imbalanced. For example, if 95% of outcomes belong to one class, a model predicting only that class will still have high accuracy. Accuracy works best when classes are evenly distributed. It should not be used alone for high-risk decisions. Use accuracy only when class distribution is balanced.

Example scenario: A model predicting whether an email is spam or not where spam and non-spam emails are evenly distributed.

Exam cue words: “Overall correctness”, “percentage of correct predictions”, “balanced dataset”

*Area Under the Curve (AUC)*

AUC measures how well a model distinguishes between classes across different threshold values. It focuses on ranking predictions rather than exact classification. A higher AUC means the model is better at separating positive and negative cases. AUC is useful when decision thresholds may change. It is commonly used in risk scoring and classification problems. If ranking or probability scoring matters, choose AUC.

Example scenario: A credit risk model ranks customers from low to high risk instead of making a strict approve/deny decision.

Exam cue words: “Ranking”, “threshold-independent”, “class separation”

*F1 Score*

The F1 score balances precision and recall into a single metric. It is especially useful when false positives and false negatives both matter. Unlike accuracy, it performs well with imbalanced datasets. F1 score is common in fraud detection, medical diagnosis, and anomaly detection. It provides a more realistic view of model performance in sensitive use cases. If both types of errors matter, choose F1 score.

Example scenario: A fraud detection system where missing fraud and falsely flagging customers are both costly.

Exam cue words: “Precision and recall”, “imbalanced data”, “false positives and negatives”

*Business Metrics (Business Evaluation)*
*Cost per user*

Cost per user measures how much it costs to serve each user with the ML system. This includes inference, infrastructure, and maintenance costs. Even an accurate model can fail if serving costs are too high. This metric helps assess scalability and sustainability. It directly links ML performance to operational expenses. If cost efficiency is emphasized, think cost per user.

Example scenario: A recommendation engine costs too much per user due to expensive real-time inference.

Exam cue words: “Operational cost”, “per-user expense”, “scalability concern”

*Development costs*

Development costs include expenses related to data collection, model training, infrastructure, and human resources. Complex models may provide better accuracy but cost significantly more to build and maintain. This metric helps organizations decide whether the performance gain justifies the expense. It is critical during cost-benefit analysis. AWS exams often test this trade-off logic. If cost vs improvement is discussed, think development costs.

Example scenario: A custom ML model performs slightly better than a pre-trained model but costs much more to develop.

Exam cue words: “Budget”, “development effort”, “cost-benefit analysis”

*Customer feedback*

Customer feedback measures how users perceive the ML system’s output. Even technically accurate models may fail if users find results confusing or unhelpful. Feedback provides qualitative insight beyond numeric metrics. It helps identify usability and trust issues. Business success depends heavily on user acceptance. If user trust or adoption is mentioned, think customer feedback.

Example scenario: Users complain that a chatbot gives correct but overly complex responses.

Exam cue words: “User satisfaction”, “feedback”, “experience”

*Return on Investment (ROI)*

ROI compares the financial benefits gained from an ML system against its total costs. It answers whether the ML solution is worth continuing or scaling. High technical performance does not guarantee high ROI. ROI is a final decision-making metric for leadership. AWS exams often frame ROI as the ultimate success indicator. If the question asks “Is this worth it?”, think ROI.

Example scenario: An ML-based recommendation system increases sales enough to justify its operating costs.

Exam cue words: “Business value”, “profit”, “financial return”

*Accuracy → balanced data; AUC → ranking quality; F1 score → imbalanced & high-risk cases; Cost per user → scalability; Development cost → feasibility; Customer feedback → adoption; ROI → final decision*
